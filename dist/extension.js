...
async function discoverAllRules() {
  ...
  try {
    ...
    for (const team of structure.teams) {
      const teamPath = path3.join(workspaceRoot, ".cursor-rules-registry", "teams", team);
    ...
    for (const user of structure.users) {
      const userPath = path3.join(workspaceRoot, ".cursor-rules-registry", "users", user);
...
async function getTeamRules(teamName) {
  ...
  const teamPath = path3.join(workspaceRoot, ".cursor-rules-registry", "teams", teamName);
...
async function getUserRules(userEmail) {
  ...
  const userPath = path3.join(workspaceRoot, ".cursor-rules-registry", "users", userEmail);
...
function getAppliedRulesDir() {
  ...
  const appliedDir = path5.join(workspaceRoot, ".cursor", "rules", "registry");
  return appliedDir;
}
...
function generateUniqueFilename(originalPath, targetDir, source) {
  ...
  while (fs3.existsSync(path5.join(targetDir, uniqueName))) {
    counter++;
...
var CursorRulesRegistryPanel = class _CursorRulesRegistryPanel {
  ...
  async handleApplyRule(ruleId) {
    ...
    try {
      ...
      if (isApplied) {
        ...
        if (success) {
          vscode5.window.showInformationMessage(`Rule "${rule.title}" has been removed.`);
          const activeTab2 = this.getActiveTab();
          if (activeTab2) {
            await this.loadTabData(activeTab2);
          }
        } else {
          vscode5.window.showErrorMessage(`Failed to remove rule "${rule.title}".`);
        }
        return;
      }
      const config = {
   ...
   */
  async handleApplyAllRules(tabName) {
    ...
    try {
      ...
      switch (tabName) {
        ...
        case "personal":
          ...
          break;
        default:
          vscode5.window.showErrorMessage(`Cannot apply all rules for tab: ${tabName}`);
          return;
      }
      if (rules.length === 0) {
        vscode5.window.showInformationMessage("No rules to apply.");
        return;
      }
      ...
      if (unappliedRules.length === 0) {
        vscode5.window.showInformationMessage("All rules are already applied.");
        return;
      }
      let successCount = 0;
      let errorCount = 0;
      for (const rule of unappliedRules) {
        try {
          ...
          error(`Failed to apply rule: ${rule.title}`, err);
          errorCount++;
        }
      }
      if (successCount > 0) {
        ...
        vscode5.window.showErrorMessage(`Failed to apply any rules.`);
      }
      await this.loadTabData(tabName);
    } catch (err) {
      error("Failed to apply all rules", err);
      vscode5.window.showErrorMessage(
